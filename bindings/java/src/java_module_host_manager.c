...
size_t JavaModuleHostManager_Size(JAVA_MODULE_HOST_MANAGER_HANDLE manager)
{
	...
	return manager == NULL ? -1 : manager->module_count;
}
...
static JAVA_MODULE_HOST_MANAGER_RESULT internal_inc_dec(JAVA_MODULE_HOST_MANAGER_HANDLE manager, int addend)
{
	...
	else
	{
		...
		else
		{
			...
			{
				/*Codes_SRS_JAVA_MODULE_HOST_MANAGER_14_019: [ The function shall return MANAGER_ERROR if the function fails to release the lock. ]*/
				LogError("Failed to release lock.");
				result = MANAGER_ERROR;
			}
			else
			{
				/*Codes_SRS_JAVA_MODULE_HOST_MANAGER_14_020: [ The function shall return MANAGER_OK if the all succeeds. ]*/
				result = MANAGER_OK;
			}
		}
	}
